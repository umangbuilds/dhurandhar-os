...
## 2026-05-06 — Defer deployment-advisor PSP scrub to 2026-05-10

**Decision:** Leave `skills/deployment-advisor/SKILL.md` PSP comparison table (lines 131–134) and operator voice sample (line 142) intact until 2026-05-10. On that date, reframe the table from named PSPs to a criteria-based PSP selection framework (DX, settlement timing, offline reach, enterprise fit) so the advisor stops naming any single player.
**Alternatives considered:** (a) Strip PSP names from the advisor today, in the same pass that scrubbed STYLE.md / voice-guide.md / CONTRIBUTING.md. (b) Keep the named comparison indefinitely.
**Rationale:** The STYLE.md scrub fixed real awkward optics — a "Razorpay banned" line in policy prose. The deployment-advisor is a different category: it leads with a Razorpay compliment ("best DX, fastest integration") and provides operational utility to founders asking "which PSP do I integrate?" Stripping it today removes a compliment and degrades the skill. Reframe is scheduled for 2026-05-10 — when it lands, it should be criteria-based, not name-based, so future MoltPe positioning doesn't conflict with the advisor.
